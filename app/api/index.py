...
# Resolve the directory this file lives in (works locally AND on Vercel)
...
DATABASE = os.path.join('/tmp', 'nchds.db') if os.environ.get('VERCEL') else 'nchds.db'
...
def init_db():
    # On Vercel: /tmp is fresh per cold start, no need to delete
    # Locally: force rebuild to pick up schema changes
    if not os.environ.get('VERCEL') and os.path.exists(DATABASE):
        os.remove(DATABASE)
